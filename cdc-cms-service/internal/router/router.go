...
func SetupRoutes(
	...
	// T14 P4 — V2 namespace alias routes mount V1 RegistryHandler directly
	// thay vì delegate qua thin V2 method. Giữ wire URL /v1/source-objects/...
	// nhưng V2 handler chỉ chứa direct (V2-only) endpoints.
	...
	// V2 namespace direct endpoints — V2 handler tự dispatch qua bus.
	admin.Post("/v1/source-objects/register", registryHandler.Register)
	...
	// V1 bridge endpoints — mount V1 RegistryHandler trực tiếp (T14 P4 dedup,
	// trước đây có thin-delegate methods trong V2 handler đã bị xóa).
	admin.Post("/v1/source-objects/register-batch", registryHandler.BulkRegister)
	admin.Patch("/v1/source-objects/registry/:id", registryHandler.Update)
	admin.Post("/v1/source-objects/registry/:id/standardize", registryHandler.Standardize)
	admin.Post("/v1/source-objects/registry/:id/scan-fields", registryHandler.ScanFields)
	admin.Post("/v1/source-objects/registry/:id/transform", registryHandler.Transform)
	admin.Post("/v1/source-objects/registry/:id/create-default-columns", registryHandler.CreateDefaultColumns)
	admin.Post("/v1/source-objects/registry/:id/detect-timestamp-field", registryHandler.DetectTimestampField)
