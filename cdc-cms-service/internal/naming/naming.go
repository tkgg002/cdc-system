// Package naming centralizes schema/object naming conventions used by the
// CMS control-plane when emitting shadow-schema names. The prefix is sourced
// from CDC_SHADOW_SCHEMA_PREFIX so renaming the convention (e.g. "lake_",
// "raw_") is a config change, not a code change. This mirrors the worker's
// centralized-data-service/internal/naming package so both sides stay in sync.
package naming
