-- Phase multi_engine_unified — L4 MariaDB seed.
-- Mounted at /docker-entrypoint-initdb.d so it runs on first
-- container start (fresh volume only).
...
-- Debezium MySql connector requires SELECT, RELOAD, SHOW DATABASES,
-- REPLICATION SLAVE, REPLICATION CLIENT for snapshot + binlog.
